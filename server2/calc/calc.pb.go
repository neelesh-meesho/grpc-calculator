// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        v5.29.3
// source: calc/calc.proto

package calc

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type CalculateRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	A             int32                  `protobuf:"varint,1,opt,name=a,proto3" json:"a,omitempty"`
	Opr           string                 `protobuf:"bytes,2,opt,name=opr,proto3" json:"opr,omitempty"`
	B             int32                  `protobuf:"varint,3,opt,name=b,proto3" json:"b,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CalculateRequest) Reset() {
	*x = CalculateRequest{}
	mi := &file_calc_calc_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CalculateRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CalculateRequest) ProtoMessage() {}

func (x *CalculateRequest) ProtoReflect() protoreflect.Message {
	mi := &file_calc_calc_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CalculateRequest.ProtoReflect.Descriptor instead.
func (*CalculateRequest) Descriptor() ([]byte, []int) {
	return file_calc_calc_proto_rawDescGZIP(), []int{0}
}

func (x *CalculateRequest) GetA() int32 {
	if x != nil {
		return x.A
	}
	return 0
}

func (x *CalculateRequest) GetOpr() string {
	if x != nil {
		return x.Opr
	}
	return ""
}

func (x *CalculateRequest) GetB() int32 {
	if x != nil {
		return x.B
	}
	return 0
}

type CalculateResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Result        int32                  `protobuf:"varint,1,opt,name=result,proto3" json:"result,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CalculateResponse) Reset() {
	*x = CalculateResponse{}
	mi := &file_calc_calc_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CalculateResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CalculateResponse) ProtoMessage() {}

func (x *CalculateResponse) ProtoReflect() protoreflect.Message {
	mi := &file_calc_calc_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CalculateResponse.ProtoReflect.Descriptor instead.
func (*CalculateResponse) Descriptor() ([]byte, []int) {
	return file_calc_calc_proto_rawDescGZIP(), []int{1}
}

func (x *CalculateResponse) GetResult() int32 {
	if x != nil {
		return x.Result
	}
	return 0
}

var File_calc_calc_proto protoreflect.FileDescriptor

const file_calc_calc_proto_rawDesc = "" +
	"\n" +
	"\x0fcalc/calc.proto\x12\x04main\"@\n" +
	"\x10CalculateRequest\x12\f\n" +
	"\x01a\x18\x01 \x01(\x05R\x01a\x12\x10\n" +
	"\x03opr\x18\x02 \x01(\tR\x03opr\x12\f\n" +
	"\x01b\x18\x03 \x01(\x05R\x01b\"+\n" +
	"\x11CalculateResponse\x12\x16\n" +
	"\x06result\x18\x01 \x01(\x05R\x06result2P\n" +
	"\x10CalculateService\x12<\n" +
	"\tCalculate\x12\x16.main.CalculateRequest\x1a\x17.main.CalculateResponseB\aZ\x05/calcb\x06proto3"

var (
	file_calc_calc_proto_rawDescOnce sync.Once
	file_calc_calc_proto_rawDescData []byte
)

func file_calc_calc_proto_rawDescGZIP() []byte {
	file_calc_calc_proto_rawDescOnce.Do(func() {
		file_calc_calc_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_calc_calc_proto_rawDesc), len(file_calc_calc_proto_rawDesc)))
	})
	return file_calc_calc_proto_rawDescData
}

var file_calc_calc_proto_msgTypes = make([]protoimpl.MessageInfo, 2)
var file_calc_calc_proto_goTypes = []any{
	(*CalculateRequest)(nil),  // 0: main.CalculateRequest
	(*CalculateResponse)(nil), // 1: main.CalculateResponse
}
var file_calc_calc_proto_depIdxs = []int32{
	0, // 0: main.CalculateService.Calculate:input_type -> main.CalculateRequest
	1, // 1: main.CalculateService.Calculate:output_type -> main.CalculateResponse
	1, // [1:2] is the sub-list for method output_type
	0, // [0:1] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_calc_calc_proto_init() }
func file_calc_calc_proto_init() {
	if File_calc_calc_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_calc_calc_proto_rawDesc), len(file_calc_calc_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   2,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_calc_calc_proto_goTypes,
		DependencyIndexes: file_calc_calc_proto_depIdxs,
		MessageInfos:      file_calc_calc_proto_msgTypes,
	}.Build()
	File_calc_calc_proto = out.File
	file_calc_calc_proto_goTypes = nil
	file_calc_calc_proto_depIdxs = nil
}
